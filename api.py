...
def returnoutput(): 
    ...
    diseases_set=makedicfromarr(name)
    model = tf.keras.models.load_model('my_h5_model.h5')
    diseases_set=pd.DataFrame.from_dict(diseases_set)
    pred = model.predict(diseases_set)
    d['output']=(col[pred.argmax()])
    return jsonify({'name': col[pred.argmax()]})
